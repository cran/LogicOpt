...
bool PLA_verify(pPLA PLA1, pPLA PLA2)
{
	...
	printf("Warning: cannot permute columns without names\n");
	return TRUE;
    }

    if (PLA1->F->sf_size != PLA2->F->sf_size) {
	printf("PLA_verify: PLA's are not the same size\n");
	return TRUE;
    }
 ...
 */
bool check_consistency(pPLA PLA, bool check_fd)
{
    bool verify_error = FALSE;
    pcover T;

    /* some use cases don't want to ckeck for fd overlaps */
    if (check_fd) {
