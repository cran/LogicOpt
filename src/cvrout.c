...
char *fmt_cube(register pset c, register char *out_map, register char *s)
{
    ...
    for(var = cube.num_binary_vars; var < cube.num_vars - 1; var++) {
       ...
       if (single_row_mv &&
            (num_input_values_used(c, var) == cube.part_size[var])) {
           s[len++] = '-';
        }
        else {
           int k = 0, mv = 0;
	   for(i = cube.first_part[var]; i <= cube.last_part[var]; i++) {
            if (single_row_mv) {
              /*** NEED TO FIX FOR actual PLA labels!!! ***/
              /*** also need to unravel MV inputs!!! ***/
              if (is_in_set(c,i)) {
                 mv += (1 << k);
                 sprintf(label,"%d",k);
...
void print_cube(register FILE *fp, pPLA PLA, register pset c, register char *out_map)
{
