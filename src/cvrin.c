...
void skip_line(register FILE *fpin, register int echo)
{
    register int ch;
...
"declared size of variable %d (counting from variable 0) is too small\n", var);
	...
	switch (getc(fp)) {
		...
		break;
            case 'c':
                printf("QCA 'c' term not yet supported.\n");
                goto bad_char;
                break;
	    case '3': case '0':
...
bad_char:
    printf("warning): input line #%d ignored\n", lineno);
    skip_line(fp,  TRUE);
    return;
}
...
loop:
	...
	case '.':
		...
		if (cube.fullset != NULL) {
		    printf("extra .i ignored\n");
		    skip_line(fp,  /* echo */ FALSE);
		} else {
...
"num_vars (1st field of .mv) must exceed num_binary_vars (2nd field of .mv)");
		...
		if (echo_unknown_commands)
		    printf("%c%s ", ch, word);
		skip_line(fp,  echo_unknown_commands);
	    }
	    break;
	default:
	    (void) ungetc(ch, fp);
...
int read_pla(FILE *fp, int needs_dcset, int needs_offset, int pla_type, pPLA *PLA_return)
{
	...
	if (cube.part_size[third] != cube.part_size[second]) {
	    printf(" with .kiss option, third to last and second\n");
	    printf("to last variables must be the same size.\n");
	    return EOF;
	}
