...
/*
*/
int num_input_values_used(pcube c, int var)
{
   int i, tot=0;

   for(i = cube.first_part[var]; i <= cube.last_part[var]; i++)
      if (is_in_set(c,i)) tot++;
   return tot; 
}

/*
Return copy of cube c with all values for input var set to off
except for value j.
*/
static pcube copy_cube_vari(pcube c, int var, int j)
{
   int i; 
   pcube c1 = set_save(c);
   for(i = cube.first_part[var]; i <= cube.last_part[var]; i++)
      if (i != j) set_remove(c1,i);

   return c1;
}


void unravel_srmv_PLA(pPLA PLA, int output_type)
{
    if (output_type & F_type) {
       ...
       while (plaF != (pcover) NULL) {
          pcover plaFnext = plaF->sfnext;
          if (first) {
             PLA->F = unravel_srmv(plaF);
             PLA->F->sfnext = plaFnext;
             plaPrev = PLA->F;
             first = FALSE;
          }
          else {
             plaF = unravel_srmv(plaF);
             plaF->sfnext = plaFnext;
             plaPrev->sfnext = plaF; 
             plaPrev = plaF;
          }
          plaF = plaFnext;
   ...
   if (output_type & D_type)
      PLA->D = unravel_srmv(PLA->D);
   if (output_type & R_type)
      PLA->R = unravel_srmv(PLA->R);
}

/* 
Return a new pcover with all multi-value inputs expanded such that only
one value is used ineach row.  If all input values are used for some input,
leave it alone.  This will be represented as a DONTCARE in output file. 
*/
pcover unravel_srmv(pcover A)
{
    register pcube c1, c, last;
    int i, var, nval;
    pcover Avar, Atot;
    ...
    for(var = cube.num_binary_vars; var < cube.num_vars-1; var++) {
       Atot = new_cover(A->count);
       foreach_set(A, last, c) {
         nval = num_input_values_used(c,var);

         /* copy cube as is if 0, 1, or all values used */
         if ((nval <= 1) || (nval == cube.part_size[var])) {
            Avar = new_cover(1);
            Avar = sf_addset(Avar, c);
         }
         else { /* expand the cube */
            Avar = new_cover(nval);
            for(i = cube.first_part[var]; i <= cube.last_part[var]; i++) {
               if (is_in_set(c,i)) { 
                  c1 = copy_cube_vari(c, var, i);
                  Avar = sf_addset(Avar, c1);
               }
            }
         }
         
         pcube xlast, p;
         foreach_set(Avar, xlast, p) {
            Atot = sf_addset(Atot, p);
         }
         sf_free(Avar);
       }
       sf_free(A);
...
int so_do_exact(pPLA PLA, int i)
{
    ...
    EXEC_S(PLA->F = minimize_exact(PLA->F, PLA->D, PLA->R, 1,0,0), word, PLA->F);
    return 1;
}
...
int so_both_do_exact(pPLA PLA, int i)
{
    ...
    skip_make_sparse = 1;
    EXEC_S(PLA->R = minimize_exact(PLA->R, PLA->D, PLA->F, 1,0,0), word, PLA->R);
